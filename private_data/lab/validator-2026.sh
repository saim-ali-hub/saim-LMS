validate_lab1_navigation() {
    ...
    if uname -r | grep -q -Ff "$SERVER_INFO"; then
    ...
    if [ -f "$HOME_DIR/office/hobby/personal/office_tree" ] &&
       grep -q "personal" "$HOME_DIR/office/hobby/personal/office_tree"
    then
       pass "Task 14: tree output saved correctly"
    else
       fail "Task 14: tree output missing or incorrect"
    fi
    ...
    PERCENT=$((PASSED * 100 / TOTAL_TASKS))
        ...
        echo "TOTAL = $TOTAL_TASKS<br>"
        echo "PASSED = $PASSED<br>"
        echo "PERCENTAGE = $PERCENT%<br>"
        echo "</div>"
        echo "<h3>==================================</h3>"

    ## Result Table Loggingi
    RESULT_FILE="/tmp/.syslog/${LAB_NUMBER}_result.txt"

    if [ ! -f "$RESULT_FILE" ]; then
    {
        echo "Result - $LAB_NAME"
        echo "========================================================================================"
        printf "%-5s %-25s %-22s %-6s %-6s %-10s\n" "Sr.#" "Name" "Date" "Total" "Passed" "Percentage"
        echo "----------------------------------------------------------------------------------------"
     } > "$RESULT_FILE"
     fi
